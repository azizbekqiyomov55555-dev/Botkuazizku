...
def init_db():
    ...
    INSERT OR IGNORE INTO networks(name) VALUES('Telegram');
    INSERT OR IGNORE INTO networks(name) VALUES('Instagram');
    INSERT OR IGNORE INTO networks(name) VALUES('TikTok');
    INSERT OR IGNORE INTO networks(name) VALUES('YouTube');
    INSERT OR IGNORE INTO networks(name) VALUES('Facebook');
    INSERT OR IGNORE INTO networks(name) VALUES('Twitter');
    """)
    c.commit(); c.close()
...
# ══════════════════════════════════════════
#  API — XIZMATLARNI AVTOMATIK YUKLASH
# ══════════════════════════════════════════

# Tarmoq nomlari va ularning kalit so'zlari
NET_KEYWORDS = {
    "Telegram":  ["telegram"],
    "Instagram": ["instagram"],
...
async def api_get_all_services():
    """API dan barcha xizmatlarni oladi"""
    import aiohttp
...
def api_detect_networks(services: list) -> list:
    """Barcha xizmatlar ichidan mavjud tarmoqlarni aniqlaydi"""
    found = []
    all_text = " ".join(
        (str(s.get("category","")) + " " + str(s.get("name",""))).lower()
        for s in services
    )
    for net_name, keywords in NET_KEYWORDS.items():
        if any(kw in all_text for kw in keywords):
            found.append(net_name)
    return found  # ["Telegram", "Instagram", ...]

def api_filter_by_net(services: list, net_name: str) -> dict:
    """Tarmoqqa oid xizmatlarni kategoriyalar bo'yicha guruhlaydi"""
    ...
    for s in services:
        ...
        cat_l = cat.lower()
        if any(kw in cat_l or kw in name for kw in keywords):
            if cat not in cats:
                cats[cat] = []
            cats[cat].append(s)
    return cats  # {"Kategoriya nomi": [...xizmatlar...]}

async def sync_net_from_api(net_name: str) -> tuple:
    """Tarmoqqa oid xizmatlarni API dan yuklab DB ga saqlaydi"""
    try:
        import aiohttp
        apis = db().execute("SELECT * FROM apis ORDER BY id DESC LIMIT 1").fetchall()
        if not apis:
            return 0, 0
        api = apis[0]
        url = api['url'].strip()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                all_services = await resp.json(content_type=None)
        ...
        if not net:
            c.execute("INSERT INTO networks(name) VALUES(?)", (net_name,))
            net_id = c.lastrowid
        else:
            net_id = net[0]

        # Eski bo'limlarni tozalaymiz
        old_secs = c.execute("SELECT id FROM sections WHERE net_id=?", (net_id,)).fetchall()
        for sec in old_secs:
        ...
        for cat_name, srvs in cats.items():
            ...
            for s in srvs:
                ...
                desc  = str(s.get("type", "") or "")
                c.execute(
                    "INSERT INTO services(sec_id,api_id,name,price,min_q,max_q,info) VALUES(?,?,?,?,?,?,?)",
                    (sec_id, sid, sname, price, min_q, max_q, desc)
                )
                srvs_count += 1

        c.commit(); c.close()
        return secs_count, srvs_count

    except Exception as e:
        logging.error(f"sync_net_from_api xatosi ({net_name}): {e}")
        return 0, 0

async def auto_load_all_networks():
    """API kiritilgandan keyin barcha tarmoqlarni bir so'rovda yuklaydi"""
    try:
        import aiohttp
        apis = db().execute("SELECT * FROM apis ORDER BY id DESC LIMIT 1").fetchall()
        if not apis:
        ...
        if not isinstance(all_services, list) or not all_services:
            logging.error(f"API bo'sh javob qaytardi: {all_services}")
            return {}

        logging.info(f"API dan {len(all_services)} ta xizmat olindi")

        # Qaysi tarmoqlar bor ekanini aniqlaymiz
        found_nets = api_detect_networks(all_services)
        logging.info(f"Aniqlangan tarmoqlar: {found_nets}")

        results = {}
        c = db()
        for net_name in found_nets:
            keywords = NET_KEYWORDS.get(net_name, [net_name.lower()])

            # Tarmoqni DB ga qo'shamiz
            net = c.execute("SELECT id FROM networks WHERE name=?", (net_name,)).fetchone()
            if not net:
                c.execute("INSERT INTO networks(name) VALUES(?)", (net_name,))
                net_id = c.lastrowid
            ...
            for cat_name, srvs in cats.items():
                ...
                for s in srvs:
                    ...
                    price = max(1, int(rate * 12700))  # 1000 tasi uchun so'mda
                    try:    min_q = int(float(s.get("min", 10)))
                    except: min_q = 10
                    try:    max_q = int(float(s.get("max", 100000)))
                    except: max_q = 100000
            ...
            c.commit()
            results[net_name] = (secs_count, srvs_count)
            logging.info(f"{net_name}: {secs_count} bo'lim, {srvs_count} xizmat")

        c.close()
        return results

    except Exception as e:
        logging.error(f"auto_load_all_networks xatosi: {e}")
        return {}
...
# Default tarmoqlar — API bo'lmasa ham ko'rinadi
DEFAULT_NETS = ["Telegram", "Instagram", "TikTok", "YouTube", "Facebook", "Twitter"]
DEFAULT_NET_ICONS = {
    "Telegram": "✈️", "Instagram": "📷", "TikTok": "🎵",
...
def kb_nets(nets, is_adm):
    b = InlineKeyboardBuilder()
    for n in nets:
        icon = DEFAULT_NET_ICONS.get(n['name'], "🌐")
...
def ensure_default_nets():
    """Default tarmoqlarni DB ga qo'shadi (agar yo'q bo'lsa)"""
    c = db()
...
async def h_buyurtma(msg: types.Message, state: FSMContext):
    ...
    # Default tarmoqlarni qo'shamiz
    ensure_default_nets()
    nets   = get_nets()
    is_adm = msg.from_user.id in ADMIN_IDS

    if not nets:
        await msg.answer("⚠️ Tarmoqlar yuklanmadi, qayta urinib ko'ring."); return
...
async def cb_net(call: types.CallbackQuery, state: FSMContext):
    ...
    icon     = DEFAULT_NET_ICONS.get(net_name, "🌐")
    secs     = get_secs(net_id)

    # Bo'limlar yo'q → API dan yuklab ko'r
    if not secs:
        apis = db().execute("SELECT id FROM apis").fetchall()
        if apis:
            await call.message.edit_text(f"{icon} {net_name} yuklanmoqda ⏳")
            secs_c, srvs_c = await sync_net_from_api(net_name)
            secs = get_secs(net_id)

    # Bo'limlar hali ham yo'q
    if not secs:
        b = InlineKeyboardBuilder()
        if is_adm:
            b.button(text="➕ Bo'lim + Xizmat qo'shish", callback_data=f"SEC_ADD:{net_id}")
        b.button(text="◀️ Orqaga", callback_data="NET_BACK")
        b.adjust(1)
        await call.message.edit_text(
            f"{icon} {net_name}\n\n"
            f"⚠️ Xizmatlar yo'q.\n"
            f"Admin paneldan qo'lda qo'shing yoki API ulang.",
            reply_markup=b.as_markup()); return

    # Bo'limlar bor — xizmatlarni ko'rsatish
    # Agar 1 ta bo'lim bo'lsa → to'g'ri xizmatlarni ko'rsat
    if len(secs) == 1:
        srvs = get_srvs(secs[0]['id'])
        b = InlineKeyboardBuilder()
        for s in srvs:
            b.button(text=s['name'], callback_data=f"SRV:{s['id']}")
        if is_adm:
            b.button(text="🔄 Yangilash", callback_data=f"NET_SYNC:{net_id}")
        b.button(text="◀️ Orqaga", callback_data="NET_BACK")
        b.adjust(1)
        await call.message.edit_text(f"{icon} {net_name} — xizmatni tanlang:", reply_markup=b.as_markup())
        return

    # Ko'p bo'lim → bo'lim tanlash
    b = InlineKeyboardBuilder()
    for s in secs:
...
async def cb_net_sync(call: types.CallbackQuery):
    ...
    net_name = net_row[0] if net_row else ""
    icon     = DEFAULT_NET_ICONS.get(net_name, "🌐")
    await call.message.edit_text(f"⏳ {net_name} xizmatlari yangilanmoqda...")
    secs_c, srvs_c = await sync_net_from_api(net_name)
    secs = get_secs(net_id)
    b = InlineKeyboardBuilder()
    for s in secs:
    ...
    b.adjust(1)
    if srvs_c:
        txt = f"✅ {secs_c} bo'lim, {srvs_c} xizmat yuklandi!\n{icon} {net_name} — bo'limni tanlang:"
    else:
        txt = f"⚠️ {net_name} uchun API da xizmatlar topilmadi."
...
async def cb_srv(call: types.CallbackQuery, state: FSMContext):
    ...
    desc   = s['info'] if s['info'] and s['info'] not in ("Yo'q", "") else None
    ...
    txt = f"👤 {sec_nm}\n\n"
    txt += f"🔑 Xizmat IDsi: {s['api_id']}\n"
    txt += f"💵 Narxi (1000x): {s['price']} So'm\n"
    if desc:
        txt += f"ℹ️ {desc}\n"
    txt += f"\n⬇️ Minimal buyurtma: {s['min_q']} ta\n"
    txt += f"⬆️ Maksimal buyurtma: {s['max_q']} ta"
...
async def cb_buy(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"➡️ Buyurtma miqdorini kiriting:\n\n"
...
async def h_qty(msg: types.Message, state: FSMContext):
    ...
    if not srv_id:
        await state.clear()
        await msg.answer("❌ Xatolik! Qaytadan boshlang.", reply_markup=kb_main(msg.from_user.id in ADMIN_IDS)); return
    s    = get_srv(srv_id)
    if not s:
        await state.clear()
        await msg.answer("❌ Xizmat topilmadi!", reply_markup=kb_main(msg.from_user.id in ADMIN_IDS)); return
    ...
    await msg.answer(
        f"✅ {qty} saqlandi\n\n"
        f"➡️ Buyurtma havolasini kiriting:\n"
        f"📝 https://instagram.com/_ havola\n"
        f"yoki @username korinishida yuboring.",
        reply_markup=kb_back())
...
async def h_link(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        f"➡️ Malumotlarni o'qib chiqing:\n"
        f"💵 Buyurtma narxi: {price:,} so'm\n"
        f"🔗 Buyurtma manzili: {link}\n"
        f"🔢 Buyurtma miqdori: {qty}\n\n"
        f"⚠️ Malumotlar to'g'ri bo'lsa «✅ Tasdiqlash» tugmasini bosing va "
        f"sizning xisobingizdan {price:,} so'm miqdorda pul yechib olinadi va "
        f"buyurtma yuboriladibuyurtmani bekor qilish imkoni bo'lmaydi",
        reply_markup=b.as_markup())
...
async def h_back_simple(msg: types.Message, state: FSMContext):
    ...
    if cur is not None:
        # State faol — tegishli state handler o'zi hal qilsin
        # Lekin agar state handler "Orqaga" ni handle qilmasa backup sifatida:
        await state.clear()
    await msg.answer("🖥 Asosiy menyudasiz!", reply_markup=kb_main(msg.from_user.id in ADMIN_IDS))
...
async def adm_api_price(msg: types.Message, state: FSMContext):
    ...
    wait_msg = await msg.answer(
        f"✅ {domain} API qo'shildi!\n\n"
        f"⏳ Xizmatlar yuklanmoqda, kuting...",
    ...
    if results:
        ...
        for net_name, (secs_c, srvs_c) in results.items():
            ...
            total_srvs += srvs_c
        lines.append(f"\n📦 Jami: {total_srvs} ta xizmat")
        await msg.answer("\n".join(lines), parse_mode="HTML")
    else:
        await msg.answer(
            "⚠️ API dan xizmatlar yuklanmadi.\n\n"
            "Sabab:\n"
            "• API URL noto'g'ri bo'lishi mumkin\n"
            "• API kalit noto'g'ri\n"
            "• Internet muammosi\n\n"
            f"URL: {data['api_url']}"
        )
...
async def adm_net_name(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        f"Endi bu tarmoqqa bo'lim nomini kiriting\n"
        f"(masalan: Obunchi, Like, Ko'rish):",
        reply_markup=kb_back())
    await state.set_state(AdmS.sec_name)
...
async def adm_sec_name(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        f"Endi xizmat API IDsini kiriting\n"
        f"(masalan: 384):",
        reply_markup=kb_back())
    await state.set_state(AdmS.srv_api)
...
async def adm_srv_api(msg: types.Message, state: FSMContext):
    ...
    await msg.answer("💰 Xizmat narxini kiriting (1000 tasi uchun So'mda):", reply_markup=kb_back())
...
async def adm_srv_info(msg: types.Message, state: FSMContext):
    ...
    if not sec_id:
        await state.clear()
        await msg.answer("❌ Bo'lim topilmadi! Qaytadan boshlang.", reply_markup=kb_admin()); return
    api_id = data.get("srv_api", "")
    price  = data.get("srv_price", 0)
    info   = msg.text.strip()
    c      = db()
    c.execute("INSERT INTO services(sec_id,api_id,name,price,min_q,max_q,info) VALUES(?,?,?,?,100,100000,?)",
              (sec_id, api_id, api_id, price, info))
    c.commit()
    # Bo'lim va tarmoq nomini olamiz
    sec_row = c.execute(
    ...
    b.button(text="➕ Yangi tarmoq qo'shish", callback_data="NET_MORE")
