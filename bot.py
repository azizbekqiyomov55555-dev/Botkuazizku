...
# ── API dan xizmatlarni yuklash ──────────
async def fetch_api_services(net_name: str):
    """API dan tarmoq nomi bo'yicha xizmatlarni oladi"""
    import aiohttp
    apis = db().execute("SELECT * FROM apis").fetchall()
    if not apis:
        return []
    api = apis[0]  # Birinchi APIdan olamiz
    url = api['url']
    key = api['key']
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data={"key": key, "action": "services"}, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                data = await resp.json()
                # Tarmoq nomi bo'yicha filter
                net_lower = net_name.lower()
                # Telegram, Instagram, TikTok, YouTube kabi
                keywords = {
                    "telegram": ["telegram", "tg", "telega"],
                ...
                # Qaysi kalit so'zlar ishlatilsin
                search_words = [net_lower]
                for k, v in keywords.items():
                    if k in net_lower or any(k in net_lower for k in v):
                        search_words = v
                        break
                ...
                for s in data:
                    ...
                    if any(w in cat or w in name for w in search_words):
                        filtered.append(s)
                return filtered[:50]  # Max 50 ta
    except Exception as e:
        logging.error(f"API fetch error: {e}")
        return []

async def sync_api_services(net_id: int, net_name: str):
    """API xizmatlarini DB ga saqlaydi, bo'limlar bo'yicha guruhlab"""
    services = await fetch_api_services(net_name)
    if not services:
        return 0

    c = db()
    # Eski auto-sync bo'limlarini o'chirmaymiz, faqat yangilarini qo'shamiz
    # Kategoriyalarni bo'lim sifatida qo'shamiz
    categories = {}
    for s in services:
        cat = s.get("category", "Boshqa")
        if cat not in categories:
            categories[cat] = []
        categories[cat].append(s)

    count = 0
    for cat_name, srvs in categories.items():
        # Bo'lim mavjudmi?
        sec = c.execute("SELECT id FROM sections WHERE net_id=? AND name=?", (net_id, cat_name)).fetchone()
        if not sec:
        ...
        for s in srvs:
            ...
            sname  = s.get("name", "")
            rate   = float(s.get("rate", 0))   # 1000 tasi uchun narx ($)
            # So'm ga aylantirish (1 USD = API narxi * valyuta kursi)
            # Hozircha API narxini So'm da saqlaymiz (admin keyinchalik sozlaydi)
            price  = int(rate * 100)  # taxminiy, admin sozlaydi
            min_q  = int(s.get("min", 100))
            max_q  = int(s.get("max", 100000))
            desc   = s.get("description", "") or ""

            # Mavjudmi?
            exists = c.execute("SELECT id FROM services WHERE sec_id=? AND api_id=?", (sec_id, sid)).fetchone()
            if not exists:
                c.execute(
...
async def cb_net(call: types.CallbackQuery, state: FSMContext):
    ...
    # Bo'limlar yo'q va API bor → avtomatik yuklash
    if not secs:
        apis = db().execute("SELECT * FROM apis").fetchall()
        if apis:
            await call.message.edit_text(f"⏳ {net_name} xizmatlari API dan yuklanmoqda...")
            count = await sync_api_services(net_id, net_name)
            secs  = get_secs(net_id)
            if count > 0:
                pass  # yuklanди, davom etamiz
            elif not secs:
                b = InlineKeyboardBuilder()
                if is_adm:
                ...
                await call.message.edit_text(
                    f"⚠️ {net_name} uchun API dan xizmatlar topilmadi!\n"
                    f"Qo'lda qo'shishingiz mumkin.",
            ...
            b.button(text="◀️ Orqaga", callback_data="NET_BACK")
            b.adjust(1)
            await call.message.edit_text("⚠️ Bo'limlar mavjud emas! API ham ulanmagan.", reply_markup=b.as_markup()); return
...
async def cb_net_sync(call: types.CallbackQuery):
    ...
    b.adjust(1)
    await call.message.edit_text(
        f"✅ {count} ta yangi xizmat yuklandi!\n📂 {net_name} bo'limlari:",
        reply_markup=b.as_markup())
