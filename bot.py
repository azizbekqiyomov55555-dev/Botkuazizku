...
async def sync_net_from_api(net_name: str) -> tuple:
    """
    1. API dan xizmatlarni oladi
    2. net_name tarmoqqa oid xizmatlarni DB ga saqlaydi
    3. (secs_count, srvs_count) qaytaradi
    """
    api, all_services = await api_get_all_services()
    if not all_services:
        return 0, 0

    cats = api_filter_by_net(all_services, net_name)
    if not cats:
        return 0, 0

    # Tarmoqni DB ga qo'shamiz (agar yo'q bo'lsa)
    c = db()
    ...
    # Eski bo'limlar va xizmatlarni o'chirib yangilaymiz
    ...
    for cat_name, srvs in cats.items():
        ...
        for s in srvs:
            ...
            # API narxi USD/1000 → So'mga taxminiy (1 USD ≈ 12700 so'm)
            price = max(1, int(rate * 12700 / 1000 * 1000))  # 1000 tasi uchun so'mda
...
async def auto_load_all_networks():
    """API kiritilgandan keyin barcha mavjud tarmoqlarni DB ga yuklaydi"""
    api, all_services = await api_get_all_services()
    if not all_services:
        return {}
    ...
    for net_name in found_nets:
        secs, srvs = await sync_net_from_api(net_name)
        results[net_name] = (secs, srvs)
    return results  # {"Telegram": (5, 120), "Instagram": (3, 80), ...}
...
async def cb_net(call: types.CallbackQuery, state: FSMContext):
    ...
    # Bo'limlar yo'q → API dan avtomatik yukla
    if not secs:
        apis = db().execute("SELECT id FROM apis").fetchall()
        if apis:
            await call.message.edit_text(f"{icon} {net_name} xizmatlari API dan yuklanmoqda ⏳")
        ...
        if not secs:
            ...
            b.button(text="🔄 Qayta yuklash", callback_data=f"NET_SYNC:{net_id}")
            b.button(text="◀️ Orqaga", callback_data="NET_BACK")
            b.adjust(1)
            hint = "API ulanmagan yoki xizmatlar topilmadi." if not apis else "API da bu tarmoq uchun xizmatlar topilmadi."
            await call.message.edit_text(f"⚠️ {net_name}: {hint}", reply_markup=b.as_markup()); return
...
async def adm_api_price(msg: types.Message, state: FSMContext):
    ...
    wait_msg = await msg.answer(
        ...
        f"⏳ Barcha tarmoqlar (Telegram, Instagram, TikTok...) yuklanmoqda...\n"
        f"Bu bir necha soniya olishi mumkin.",
        reply_markup=kb_admin()
    )

    # API dan barcha tarmoqlarni avtomatik yuklaymiz
    results = await auto_load_all_networks()

    if results:
        lines = [f"🌐 <b>Yuklangan tarmoqlar:</b>"]
        for net_name, (secs_c, srvs_c) in results.items():
            lines.append(f"  {'📱' if net_name == 'Telegram' else '📷' if net_name == 'Instagram' else '🎵' if net_name == 'TikTok' else '▶️' if net_name == 'YouTube' else '🌐'} {net_name}: {secs_c} bo'lim, {srvs_c} xizmat")
        summary = "\n".join(lines)
        await msg.answer(summary, parse_mode="HTML")
    else:
        await msg.answer(
            "⚠️ API dan tarmoqlar yuklanmadi.\n"
            "API URL va kalit to'g'riligini tekshiring.",
        )
...
async def adm_srv_info(msg: types.Message, state: FSMContext):
    ...
    if not sec_id:
        await state.clear()
        await msg.answer("❌ Bo'lim IDsi topilmadi! Qaytadan boshlang.", reply_markup=kb_admin()); return
    ...
    c.execute("INSERT INTO services(sec_id,api_id,name,price,min_q,max_q,info) VALUES(?,?,?,?,100,100000,?)",
              (sec_id, api_id, api_id, price, msg.text.strip()))
    c.commit(); c.close()
    b = InlineKeyboardBuilder()
    ...
    b.adjust(1)
    await msg.answer(
        f"✅ Xizmat muvaffaqiyatli qo'shildi!\n\n"
        f"📦 API ID: {api_id}\n"
        f"💰 Narxi: {price} So'm\n\n"
        f"⬇️ Yana xizmat qo'shish uchun tugmani bosing.",
        reply_markup=b.as_markup())
    await state.clear()
