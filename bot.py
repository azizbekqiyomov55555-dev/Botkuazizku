...
async def cb_ch_add(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("Kanal ID'sini kiriting (@kanal yoki -100...):", reply_markup=kb_back())
...
async def adm_ch_url(msg: types.Message, state: FSMContext):
    ...
    c.execute("INSERT INTO channels(cid,title,url) VALUES(?,?,?)",
              (data["ch_cid"], data["ch_title"], msg.text.strip()))
    c.commit(); c.close()
    await msg.answer(f"✅ {data['ch_title']} kanali qo'shildi!", reply_markup=kb_admin())
...
async def adm_api_key(msg: types.Message, state: FSMContext):
    ...
    await msg.answer("✅ API kaliti saqlandi!", reply_markup=kb_admin())
...
async def adm_net_name(msg: types.Message, state: FSMContext):
    if msg.text == "Orqaga":
        await state.clear(); await msg.answer("🖥 Asosiy menyudasiz!", reply_markup=kb_main(msg.from_user.id in ADMIN_IDS)); return
    name = msg.text.strip()
