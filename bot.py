...
def gorder(oid):
    c = db()
    r = c.execute("""
        SELECT o.id, o.uid, o.srv_id, o.link, o.qty, o.price, o.status, o.at,
               s.name, n.name, sec.name
        FROM orders o
    ...
    c.close(); return r
    # 0=id,1=uid,2=srv_id,3=link,4=qty,5=price,6=status,7=at,8=srv_name,9=net_name,10=sec_name
...
async def cb_buy(call: types.CallbackQuery, state: FSMContext):
    ...
    s = gsrv(srv_id)
    if not s:
        await call.answer("Xizmat topilmadi!", show_alert=True); return
    await state.update_data(srv_id=srv_id)
    info   = sec_names(s[1])
    net_nm = info[1] if info else ""
    sec_nm = info[0] if info else ""
...
async def h_ord_link(msg: types.Message, state: FSMContext):
    ...
    for aid in ADMIN_IDS:
        try:
            o = gorder(oid)
            # o = (id,uid,srv_id,link,qty,price,status,at, srv_name, net_name, sec_name)
            net_nm = o[9] if o else ""
            sec_nm = o[10] if o else ""
...
async def adm_ord_search(msg: types.Message, state: FSMContext):
    ...
    # gorder: (id,uid,srv_id,link,qty,price,status,at, srv_name,net_name,sec_name)
    se = {"Bajarilmoqda":"⏳","Bajarildi":"✅","Bekor qilindi":"❌"}.get(o[6],"❓")
    b = InlineKeyboardBuilder()
